...
##### 1、（对 Android 开发而言）不用 `findViewById` ，可以直接使用控件的 id 进行操作，减少原先在 Java 中大量的 `findViewById` 的代码。
``` xml
...
##### 5、空安全
###### 可空类型与非空类型
> 许多编程语言（包括 Java）中最常见的异常之一，就是访问空引用的成员会导致空引用异常（`NullPointerException`）。

Kotlin 系统类型区分一个引用可以容纳 `null` 还是不能容纳，如：
``` java
...
（5）安全的类型转换
如果对象不是目标类型，那么常规类型转换可能会导致 `ClassCastException`。 使用安全的类型转换，如果尝试转换不成功则返回 `null`：
``` java
...
抽象 Builder 类，定义了三个抽象方法，用于设置产品属性及获取实例。
``` java
...
Director 类，负责构造 Mobile，通过设置的建造者，创建产品实例
``` java
